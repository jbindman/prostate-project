...
plotVectorized<-function(pt.id = 30, toCompare = c(5, 10, 20), what.data="both", log.scale=T, plot.psad=F){
#plots from separated dataframes out of ptDataframes list, as opposed to printSubset function


#subset PSA data
psa.data.i <- ptDataframes$psa.data[ptDataframes$psa.data$id==pt.id,]  #individual PSA
psa.data.compare <- dplyr::filter(ptDataframes$psa.data, id %in% toCompare) #group PSAs

#subset bx data
bx.data.i<-ptDataframes$bx.full[ptDataframes$bx.full$id==pt.id,] #individual bx
bx.data.compare <- dplyr::filter(ptDataframes$bx.full, id %in% toCompare) #group bx
