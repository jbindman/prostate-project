...
getPredictions <-function(id_i = 657, date_pred = c(today - 3*365, today - 2*365, today - 365, today), pt = ptDataframes){
  ...
  #date_pred <- c(today - 2*365, today - 365, today) #added today, last year, 2 years ago
  ...
  #50%
  percentRC <-prc_est[2,] #rc likelihood

  predictions <-   as.data.frame(cbind(age_i,percentRC))
