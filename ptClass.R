...
ptDataload <- function(txtest.data = tx.data, demotest.data = demo.data, 
                       bxtest.data = bx.data, ptlocal.data = pt.data) { #default file names
  
  #define true GS
  ptlocal.data$true.gs<-rep(NA,n) #post-surgery true GS observation; want it to be "NA" for patients without surgery
  
  for(i in 1:n){
    if(ptlocal.data$id[i]%in%tx.data$id){
      ptlocal.data$true.gs[i]<-tx.data$GS[tx.data$id==ptlocal.data$id[i]] #tx.data$GS[tx.data$id==pt.data$id[i]]
  ...
  as.Date(demotest.data$dob[1:10])
  # ADD CUSTOMIZABLE
  pt.data$dob.num<-as.numeric(as.Date(demotest.data$dob)) #demo.data$dob
